  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        setUser(data.user as HeaderUser);
        setLoadingUser(false);
      } catch {
        setLoadingUser(false);
      }
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 bg-gray-200 dark:bg-gray-700">
          {user?.imageUrl && user.imageUrl.startsWith('http') ? (
            ...
            <span className="flex items-center justify-center w-full h-full text-sm font-medium text-gray-500 dark:text-gray-400">
      ...
      >
        <div>
          ...
          <span className="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">
            {user?.email || (loadingUser ? "Loading..." : "Email not set")}
          </span>
        </div>
